...
TableFunction MySQLTableEntry::GetScanFunction(ClientContext &context, unique_ptr<FunctionData> &bind_data) {
	...
	bool use_predicate_analyzer = GetBoolSetting(context, "mysql_experimental_filter_pushdown");
	result->use_predicate_analyzer = use_predicate_analyzer;
	bind_data = std::move(result);

	auto function = MySQLScanFunction();
	function.filter_pushdown = use_predicate_analyzer || GetBoolSetting(context, "mysql_simple_filter_pushdown");
	return function;
}
